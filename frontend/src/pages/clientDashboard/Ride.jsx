...
// //       try {
// //         const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/rides/nearby`);
...
// //     socket.on('driverOffer', (driverData) => {
// //       setInterestedDrivers((prev) => {
// //         const exists = prev.find(d => d._id === driverData.driver._id);
// //         if (exists) {
// //           return prev.map(d => d._id === driverData.driver._id ? driverData.driver : d);
// //         }
// //         return [...prev, driverData.driver];
// //       });
// //       toast.info(`${driverData.driver.firstName} has proposed a price of ₦${driverData.offeredPrice}`, {
// //         style: { background: '#2196F3', color: 'white' }
// //       });
// //     });

// //     socket.on('rideConfirmed', ({ driver, passenger }) => {
// //       setDriverDetails({
...
// //         offeredPrice: driver.finalPrice || rideForm.calculatedPrice,
// //       });
// //       setRideStarted(true);
// //       setEta(driver.eta || '5 minutes');
...
// //     socket.on('driverOfferRejected', ({ driverId }) => {
// //       setInterestedDrivers((prev) => prev.filter((d) => d._id !== driverId));
// //       setShowDriverModal(false);
// //       setSelectedDriver(null);
// //       toast.info('Driver offer rejected', {
// //         style: { background: '#2196F3', color: 'white' }
// //       });
// //     });

// //     socket.on('rideCompleted', ({ rideId, rideDuration }) => {
// //       setRideProgress(100);
// //       setRideStatus('Ride completed');
// //       setRideStarted(false);
// //       toast.success(`Ride completed in ${rideDuration} minutes`, {
// //         style: { background: '#4CAF50', color: 'white' }
// //       });
// //     });
...
// //       try {
// //         const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/rides/history`, {
// //           headers: { Authorization: `Bearer ${token}` },
// //         });
...
// //   const calculateDistanceAndFare = async () => {
// //     if (!rideForm.pickupAddress.trim() || !rideForm.destinationAddress.trim()) {
// //       toast.error("Please enter valid pickup and destination addresses", { style: { background: "#F44336", color: "white" } });
// //       return;
// //     }
// //     try {
// //       const response = await axios.post(
// //         `${import.meta.env.VITE_BACKEND_URL}/api/rides/calculate-fare`,
// //         {
// //           pickupAddress: rideForm.pickupAddress,
// //           destinationAddress: rideForm.destinationAddress,
...
// //         { headers: { Authorization: `Bearer ${token}` } }
// //       );
// //       const { distance, calculatedPrice } = response.data;
// //       setRideForm((prev) => ({
// //         ...prev,
// //         distance,
// //         calculatedPrice,
...
// //       setShowMap(true);
// //     } catch (error) {
// //       const errorMessage = error.response?.data?.error || "Error calculating fare";
// //       toast.error(errorMessage, { style: { background: "#F44336", color: "white" } });
// //       console.error('Error calculating fare:', error.response?.data || error);
// //       setRideForm((prev) => ({ ...prev, distance: "", calculatedPrice: "" }));
// //     }
...
// //     try {
// //       await axios.post(
// //         `${import.meta.env.VITE_BACKEND_URL}/api/rides/${deliveryId}/chat`,
// //         { text: newMessage },
// //         { headers: { Authorization: `Bearer ${token}` } }
// //       );
// //       setChatMessages([...chatMessages, { sender: passengerId, text: newMessage }]);
// //       setNewMessage('');
// //     } catch (error) {
...
// //       toast.error('Rating must be between 1 and 5', { style: { background: '#F44336', color: 'white' } });
// //       return;
// //     }
// //     try {
// //       await axios.post(
// //         `${import.meta.env.VITE_BACKEND_URL}/api/rides/${deliveryId}/rate`,
// //         { rating, review },
// //         { headers: { Authorization: `Bearer ${token}` } }
// //       );
// //       toast.success('Rating and review submitted', { style: { background: '#4CAF50', color: 'white' } });
// //     } catch (error) {
// //       console.error('Error submitting rating/review:', error);
// //       toast.error('Failed to submit rating/review', { style: { background: '#F44336', color: 'white' } });
// //     }
// //   };
...
// //         {/* Map Section */}
// //         <div className="lg:w-2/3 w-full  lg:rounded-2xl shadow-lg overflow-hidden">
// //           {currentLocation ? (
// //             <iframe
// //               width="100%"
// //               height="100%"
...
//   const [selectedDriver, setSelectedDriver] = useState(null);
//   const [showDriverModal, setShowDriverModal] = useState(false);
//   const [acceptedDriverId, setAcceptedDriverId] = useState(null); // Track accepted driver
...
//         const exists = prev.find(d => d._id === driverData.driver._id);
//         if (exists) {
//           return prev.map(d => d._id === driverData.driver._id ? { ...driverData.driver, driverProposedPrice: driverData.offeredPrice } : d);
//         }
//         return [...prev, { ...driverData.driver, driverProposedPrice: driverData.offeredPrice }];
//       });
//       toast.info(`${driverData.driver.firstName} has proposed a price of ₦${driverData.offeredPrice}`, {
//         style: { background: '#2196F3', color: 'white' }
//       });
...
//       });
//       setAcceptedDriverId(driver._id);
//       setRideStarted(true);
//       setEta(driver.eta || '5 minutes');
//       setInterestedDrivers([]);
//       setShowDriverModal(false);
...
//   const handleAcceptDriver = async (driverId) => {
//     if (acceptedDriverId) {
//       toast.error('You have already accepted a driver', { style: { background: '#F44336', color: 'white' } });
//       return;
//     }
//     try {
//       setLoading(true);
...
//                 <button
//                   onClick={() => handleAcceptDriver(selectedDriver._id)}
//                   className={`flex-1 py-2 rounded-lg font-semibold transition-colors ${theme === 'light' ? 'bg-customGreen text-white hover:bg-customGreen' : 'bg-customGreen text-white hover:bg-customGreen'} ${loading || acceptedDriverId ? 'opacity-50 cursor-not-allowed' : ''}`}
//                   disabled={loading || acceptedDriverId}
//                 >
//                   {loading ? 'Processing...' : 'Accept Driver'}
//                 </button>
//                 <button
...
//                     <FaCar className={`text-4xl animate-car ${theme === 'light' ? 'text-customGreen' : 'text-GreenColor'}`} />
//                     <p className={`mt-2 text-sm ${theme === 'light' ? 'text-gray-600' : 'text-gray-300'}`}>
//                       Waiting for interested drivers...
//                     </p>
//                   </div>
//                 ) : (
//                   <ul className="space-y-3 mt-2">
...
//                     <p>Your Offered Price: <span className={`font-medium ${theme === 'light' ? 'text-gray-800' : 'text-white'}`}>₦{rideForm.desiredPrice}</span></p>
//                   )}
//                   {driverDetails.offeredPrice && (
//                     <p>Driver's Price: <span className={`font-medium ${theme === 'light' ? 'text-gray-800' : 'text-white'}`}>₦{driverDetails.offeredPrice}</span></p>
//                   )}
//                   <p className="mt-2">
//                     Final Price: <span className={`font-semibold ${theme === 'light' ? 'text-customGreen' : 'text-GreenColor'}`}>
//                       ₦{driverDetails.offeredPrice || rideForm.desiredPrice || rideForm.calculatedPrice}
//                     </span>
//                   </p>
//                 </div>
//                 <button
...
//                 <div className={`border p-3 rounded-lg h-24 overflow-y-auto text-sm ${theme === 'light' ? 'border-gray-200' : 'border-gray-500'}`}>
//                   {chatMessages.map((msg, index) => (
//                     <p key={index} className={msg.sender === passengerId ? 'text-right text-customGreen' : `text-left ${theme === 'light' ? 'text-gray-600' : 'text-gray-300'}`}>
...
function Ride() {
  ...
  useEffect(() => {
    ...
    socket.on('rideConfirmed', ({ driver, passenger }) => {
      ...
      setInterestedDrivers([]);
      setShowDriverModal(false);
      setShowWaitingModal(false); // Close waiting modal on driver assignment
      toast.success(`${driver.firstName} has been assigned to your ride`, {
        style: { background: '#4CAF50', color: 'white' }
      });
    });
    ...
    socket.on('rideCompleted', ({ rideId, rideDuration }) => {
      ...
      setRideStatus('Ride completed');
      setRideStarted(false);
      setShowWaitingModal(false); // Ensure modal is closed on ride completion
      toast.success(`Ride completed in ${rideDuration} minutes`, {
        style: { background: '#4CAF50', color: 'white' }
      });
    });
  ...
  useEffect(() => {
    // Show waiting modal when ride is booked but not started
    if (deliveryId && !rideStarted) {
  ...
  const handleCancelRide = async () => {
    try {
      ...
      setInterestedDrivers([]);
      setAcceptedDriverId(null);
      setShowWaitingModal(false); // Close waiting modal on cancel
      toast.success("Ride cancelled successfully", { style: { background: "#4CAF50", color: "white" } });
    } catch (error) {
      const errorMessage = error.response?.data?.error || "Failed to cancel ride";
      toast.error(errorMessage, { style: { background: "#F44336", color: "white" } });
  ...
  return (
    <div className={`font-sans flex flex-col ${theme === 'light' ? 'bg-gray-100' : 'bg-gray-900'}`}>
      ...
      {/* Waiting for Drivers Modal */}
      {showWaitingModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={`rounded-2xl p-6 w-11/12 max-w-md ${theme === 'light' ? 'bg-white shadow-lg' : 'bg-gray-700'}`}>
            <div className="flex justify-between items-center mb-4">
              <h2 className={`text-lg font-semibold ${theme === 'light' ? 'text-gray-800' : 'text-white'}`}>Finding Your Driver</h2>
              <button
            ...
            <div className="flex flex-col items-center">
              <div className="w-full h-16 overflow-hidden">
                <FaCar
                  className={`text-4xl animate-car ${theme === 'light' ? 'text-customGreen' : 'text-GreenColor'}`}
                />
              </div>
              <p className={`mt-4 text-sm font-medium text-center ${theme === 'light' ? 'text-gray-600' : 'text-gray-300'}`}>
                Waiting for available drivers to accept your ride...
              </p>
              <div className="mt-4 flex space-x-2">
                <span className={`inline-block h-2 w-2 rounded-full animate-pulse ${theme === 'light' ? 'bg-customGreen' : 'bg-GreenColor'}`}></span>
                <span className={`inline-block h-2 w-2 rounded-full animate-pulse delay-200 ${theme === 'light' ? 'bg-customGreen' : 'bg-GreenColor'}`}></span>
                <span className={`inline-block h-2 w-2 rounded-full animate-pulse delay-400 ${theme === 'light' ? 'bg-customGreen' : 'bg-GreenColor'}`}></span>
              </div>
            </div>
          </div>
        </div>
