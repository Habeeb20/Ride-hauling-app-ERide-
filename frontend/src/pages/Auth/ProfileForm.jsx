...
const ProfileForm = ({ userId }) => {
  ...
  const validateForm = (values) => {
    ...
    if (role === "driver") {
      ...
      else if (isNaN(values.YOE) || values.YOE < 0)
        errors.YOE = "Years of experience must be a non-negative number";
      if (!values.currentLocation)
        errors.currentLocation = "Current location is required";
      if (!values.languageSpoken)
        errors.languageSpoken = "Language spoken is required";
      if (!values.gearType) errors.gearType = "Gear type is required";
      if (!values.vehicleType) errors.vehicleType = "Vehicle type is required";
      if (!values.driverRoles.length)
        errors.driverRoles = "At least one driver role is required";
      if (typeof values.interstate !== "boolean")
        errors.interstate = "Interstate travel preference is required";
      ...
      if (values.driverRoles.includes("hired")) {
        ...
        else if (!["day", "days", "week", "weeks", "month", "months", "permanent", "temporary"].includes(hiredDetails.durationType))
          errors["availableToBeHiredDetails.durationType"] = "Invalid duration type";
        
        if (["day", "days", "week", "weeks", "month", "months"].includes(hiredDetails.durationType)) {
          if (!hiredDetails.durationValue || hiredDetails.durationValue < 1)
            errors["availableToBeHiredDetails.durationValue"] = "Duration value must be at least 1";
          if (!hiredDetails.endDate)
            errors["availableToBeHiredDetails.endDate"] = "End date is required for non-permanent duration";
        }

        if (!hiredDetails.minSalary)
          errors["availableToBeHiredDetails.minSalary"] = "Minimum salary is required";
        else if (hiredDetails.minSalary < 0)
          errors["availableToBeHiredDetails.minSalary"] = "Minimum salary must be non-negative";

        if (typeof hiredDetails.interstateTravel !== "boolean")
          errors["availableToBeHiredDetails.interstateTravel"] = "Interstate travel preference is required";

        if (!hiredDetails.typeOfCar)
          errors["availableToBeHiredDetails.typeOfCar"] = "Type of car is required";
        else if (!["car", "jeep", "mini-bus", "bus", "trailer"].includes(hiredDetails.typeOfCar))
          errors["availableToBeHiredDetails.typeOfCar"] = "Invalid car type";

        if (!hiredDetails.typeOfTransmission)
          errors["availableToBeHiredDetails.typeOfTransmission"] = "Type of transmission is required";
        else if (!["automatic", "manual", "both"].includes(hiredDetails.typeOfTransmission))
          errors["availableToBeHiredDetails.typeOfTransmission"] = "Invalid transmission type";

        if (!hiredDetails.choice)
          errors["availableToBeHiredDetails.choice"] = "Choice is required";
        else if (!["private", "commercial"].includes(hiredDetails.choice))
          errors["availableToBeHiredDetails.choice"] = "Choice must be private or commercial";
        ...
        // else if (!/^[0-1]?[0-9]:[0-5][0-9] (AM|PM)$/.test(hiredDetails.timeToStart))
        //   errors["timeToStart"] = "Invalid time format (e.g., 09:00 AM)";
      }
    }
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    if (Object.keys(errors).length > 0) {
      ...
      const errorMessages = {
        ...
        "availableToBeHiredDetails.durationType": "Duration Type",
        "availableToBeHiredDetails.durationValue": "Duration Value",
        "availableToBeHiredDetails.endDate": "End Date",
        "availableToBeHiredDetails.minSalary": "Minimum Salary",
        "availableToBeHiredDetails.interstateTravel": "Interstate Travel (Hired)",
        "availableToBeHiredDetails.typeOfCar": "Type of Car",
        "availableToBeHiredDetails.typeOfTransmission": "Type of Transmission",
        "availableToBeHiredDetails.choice": "Choice",
        "availableToBeHiredDetails.startDate": "Start Date",
      ...
      const firstErrorField = Object.keys(errors)[0];
      const fieldElement = firstErrorField.includes(".")
    ...
    try {
      ...
      if (role === "client" && values.question === "student" && values.schoolId) {
        schoolIdUrl = await uploadToCloudinary(values.schoolId);
      }
      if (role === "driver") {
        carPictureUrl = await uploadToCloudinary(values.carPicture);
      ...
      const body = {
        userEmail,
        gender: values.gender,
        location: { ...values.location, coordinates },
        phoneNumber: values.phoneNumber,
        profilePicture: profilePictureUrl,
        question: role === "client" ? values.question : undefined,
        schoolIdUrl,
        carPicture: carPictureUrl,
        driverLicense: driverLicenseUrl,
        certificateTraining: certificateTrainingUrl,
        maritalStatus: role === "driver" ? values.maritalStatus : undefined,
        YOE: role === "driver" ? parseInt(values.YOE) : undefined,
        currentLocation: role === "driver" ? values.currentLocation : undefined,
        languageSpoken: role === "driver" ? values.languageSpoken : undefined,
        gearType: role === "driver" ? values.gearType : undefined,
        vehicleType: role === "driver" ? values.vehicleType : undefined,
        carDetails: role === "driver" ? values.carDetails : undefined,
        driverRoles: role === "driver" ? values.driverRoles : undefined,
        interstate: role === "driver" ? values.interstate : undefined,
        availableToBeHiredDetails: role === "driver" && values.driverRoles.includes("hired")
          ? {
            ...values.availableToBeHiredDetails,
            durationValue: parseInt(values.availableToBeHiredDetails.durationValue) || undefined,
            minSalary: parseInt(values.availableToBeHiredDetails.minSalary) || 0,
            endDate: values.availableToBeHiredDetails.endDate || undefined,
          }
          : undefined,
      };
      ...
      navigate(data.isDriver ? "/login" : "/login");
    } catch (error) {
      console.error("Error submitting form:", error);
      toast.error(
        error.response?.data?.message || "Failed to create profile. Please try again.",
  ...
  const geocodeLocation = (state, lga) => {
    const mockCoordinates = {
      // Placeholder: update with actual coordinates if available
    };
    const key = `${state}-${lga}`;
    setCoordinates(mockCoordinates[key] || { lat: 6.5244, lng: 3.3792 });
  };
